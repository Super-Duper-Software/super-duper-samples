// The production `audioRenderRunner` (ticket 02, ADR-0005). Shells out to the
// bundled `ffmpeg-static` binary in ONE invocation per export — trim, encode,
// resample, downmix and loudness-normalise all happen in a single command, as
// the ticket requires. Lives in `src/main`; the core only ever sees the
// `AudioRenderRunner` function type (CONVENTIONS.md: no binary spawn in
// `src/core`).
//
// Packaging / signing the `ffmpeg-static` binary for a shipped build is v1
// ticket 19 — not here. Development spawns the binary straight out of
// `node_modules`.
...
function buildArgs(input: AudioRenderInput): string[] {
  ...
  // "Exporting the whole file with no format change is still permitted" —
  // when nothing about the encoding is actually changing, skip re-encoding.
