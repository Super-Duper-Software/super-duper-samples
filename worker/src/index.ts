...
// CORS
//
// The desktop app calls this from the Electron main process (Node fetch, no
// browser origin), so CORS is not strictly required. We still send explicit,
// permissive headers and answer OPTIONS preflight so a renderer-side call works
// in dev.
// ---------------------------------------------------------------------------
...
// Rate limiting
//
// Best-effort in-memory token bucket, keyed by client IP. A Worker runs across
// many isolates in many colos, so this Map is NOT global and NOT durable — it
// only blunts a burst from a single isolate. A production deploy should enforce
// rate limiting with a Durable Object or Cloudflare rate-limiting rules. Kept
// here because it holds nothing user-specific and is dropped on isolate
// recycle, so the Worker stays stateless in every meaningful sense.
// ---------------------------------------------------------------------------
...
async function exchangeWithFreesound(
  ...
  try {
    ...
    // Network failure reaching Freesound — always retryable.
    return json(503, {
      error: "retry",
      upstream_status: 0,
  ...
  if (!upstream.ok) {
    ...
    // 5xx and 429 are transient; retry. Everything else 4xx (invalid_grant,
    // expired code, revoked/expired refresh token, bad client) means the grant
    // is dead and the user must re-authorize.
...
const worker = {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === "/exchange") {
      ...
      form.set("code", code);
      // Freesound's token call does not require redirect_uri, but forward it if
      // the client sent one.
      if (
