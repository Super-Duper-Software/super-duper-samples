// The app's own log (ticket 18). Errors that reach the user are ALSO written
// here so a bug report has something concrete behind it — "errors are surfaced
// in the interface, not only in logs" does not mean the log goes away, it means
// the log stops being the ONLY place.
//
// The core stays Electron-free: a `LogSink` is the boundary. Production wires a
// file sink (`createFileLogSink`) from `src/main`; tests inject a memory sink or
// use `NULL_LOG_SINK`. The renderer never touches this directly — it calls
// `core.log(...)` / `core.readLog()` over the bridge.
