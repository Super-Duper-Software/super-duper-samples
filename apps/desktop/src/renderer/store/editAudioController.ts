// The Edit view's OWN audio-source boundary (ticket 07). Deliberately separate
// from `audioController.ts`, which is scoped to streaming a Sound's Preview for
// row auditioning (ADR-0003) and must never be repurposed to play a local file.
//
// This module plays the Sound's local ORIGINAL directly — the file the Edit
// view is trimming — via a `file://` URL, and loops playback over an
// arbitrary `[startSec, endSec]` region.
//
// The loop-back seek AND the playhead are both driven by a 60fps
// requestAnimationFrame loop, not the browser's `timeupdate` event (which
// fires only a few times a second — plenty for a progress bar, but it let a
// tight region overshoot `endSec` by up to ~250ms before looping back, and
// made the playhead visibly stutter). The playhead position is written
// straight onto a registered DOM node's `--playhead` custom property, the
// same technique `audioController.ts` uses for the row-list playhead, so
// advancing playback never triggers a React re-render.
//
// Entirely independent of the row-list transport: opening the Edit view never
// touches `useTransport`, and this element is torn down when the view closes.
...
export function loadRegion(
  ...
  region = r
  if (!a) return
  a.loop = false // looping is done by hand against `region`, not the native attribute
  // Always re-`src` + `.load()`, even when the URL string is unchanged: an
  // Edit's negative id — and so its exported file's path — gets REUSED once a
  // deleted Edit's id frees up (ADR-0005's `nextEditId`). A same-path skip
  // here left this element quietly holding on to the PREVIOUS Edit's decoded
  // audio the first time the reused path was played, even though the on-disk
  // bytes underneath it had already changed.
