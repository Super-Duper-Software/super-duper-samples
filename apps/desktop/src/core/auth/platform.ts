// `AuthPlatform` — the small collaborator interface for the three capabilities
// the core genuinely cannot provide for itself, in the same spirit as
// `FreesoundGateway` and `DragHost`:
//
//   (a) opening the user's real system browser,
//   (b) a one-shot loopback HTTP listener that captures the OAuth callback,
//   (c) OS-backed encryption (`safeStorage`) for the refresh token at rest.
//
// Everything else about sign-in — building the authorize URL, generating and
// verifying `state`, exchanging the code, storing tokens, the 401 interceptor,
// proactive-refresh scheduling, the signed-in/out state machine — is core code
// and is tested with no Electron (a fake AuthPlatform + a fake Scheduler).
//
// `keytar` is NOT used anywhere: encryption is Electron's `safeStorage`, reached
// only through `encrypt`/`decrypt` below.
