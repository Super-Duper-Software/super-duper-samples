// Staging eviction, disk accounting and on-demand clear (ticket 10).
//
// Staged Originals are speculative downloads the user auditioned but never
// saved. They must not be allowed to fill the disk, so total Staged bytes are
// kept under a configurable budget by evicting the LEAST-recently-accessed
// Staged Sounds first (`staged_entries` is read `ORDER BY last_access_at ASC`).
//
// Two Sounds are never evicted:
//   - one with a `library_entries` row — the user chose to keep it (CONTEXT.md
//     § Library); it is not Staged material at all;
//   - one with a live Drag-Out in progress (`InFlightDrags`) — the OS may still
//     be reading the path we handed it.
//
// A Sound is removed as a unit: its Original, its mandatory sidecar (ADR-0002)
// and its `staged_entries` row, leaving no orphan of either kind. The Original
// is unlinked FIRST, so an interrupted eviction can only ever leave a harmless
// sidecar-without-audio — never audio without its identity, which is the
// invariant `writeOriginal` establishes by renaming the sidecar into place
// before the Original.
//
// All filesystem work is async (`node:fs/promises`) so a large eviction never
// blocks the event loop; the caller runs it off the hot path (see
// `stagingController`).
...
function pathsFor(
  ...
  // Older rows (pre-ticket-08) carry no `path`; re-derive it from the Sound.
...
async function removeStaged(
  ...
  // Original first — a crash here leaves only a harmless orphan sidecar.
  await rm(original, { force: true })
  await rm(sidecar, { force: true })
...
export async function evictStagedOverBudget(
  ...
  const entries = listStagedEntries(db) // LRU-first
