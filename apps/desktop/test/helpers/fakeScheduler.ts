// Deterministic `Scheduler` for auth tests. Time only moves when `advance()` is
// called; due callbacks fire in order, and callbacks scheduled by a callback
// (proactive refresh reschedules itself) are picked up within the same advance.
