...
describe('core.search pagination', () => {
  ...
  it('walks page N and returns a stable result shape each page', async () => {
    ...
    // Envelope shape is stable and self-describing on every page.
    ...
    // Page 1 has more; page 2 is the last page.
    expect(p1.hasMore).toBe(true)
    expect(p2.hasMore).toBe(false)

    // Both pages carry the full row field set, and the same set.
    expect(p1.sounds).toHaveLength(3)
    expect(p2.sounds).toHaveLength(3)
    expect(rowKeys(p2.sounds[0]!)).toEqual(rowKeys(p1.sounds[0]!))

    // Appending page 2 onto page 1 yields six distinct Sounds — no overlap.
