// The result-list selection model, exposed as a tiny Zustand store so that
// ticket 04 (play / prev / next) can read and drive it without this ticket's
// components re-rendering the transport.
//
// Spec 0001 names Zustand as the renderer's state tool. This store holds ONLY
// "which row is selected" — no playback state, no per-scroll-frame state.
