// Ticket 13 — Library organisation, at the core seam. No Electron: real temp
// SQLite, real temp filesystem, the fake gateway (recording), a recording
// `DragHost` for the drag-name assertions.
//
// Covers the ticket's "Tests cover:" line:
//   - a custom name flows through to the path handed to `DragHost`
//   - renaming preserves author, License and Freesound linkage
//   - unsafe characters in a custom name are sanitised (same scheme as ticket 09)
//   - filtering by each dimension returns correct results and makes NO gateway call
// plus: custom names + tags survive a restart; the free-text filter matches
// custom name / Freesound name / author / tags; the persisted filter round-trips.
...
const RAIN_ID = 321967 // Rain_Heavy_Loop.wav, in search-rain.json
...
describe('a custom name flows through to the Drag-Out path', () => {
  it('the file handed to DragHost carries the user’s name, not the Freesound name', async () => {
    ...
    // still the Original's bytes, just under the user's name
  ...
  it('sanitises unsafe characters in a custom name and keeps collisions disambiguated', async () => {
    ...
    // none of the macOS/Windows-illegal characters survive
    expect(name).not.toMatch(/[/\\:*?"<>|]/)
    expect(name.startsWith('Bad Name')).toBe(true)

    // re-dragging the SAME renamed Original reuses the one hardlink (idempotent),
    // so the `(2)` scheme is still reserved for genuine collisions.
    const again = core.startDrag(RAIN_ID)
    expect(again.filePath).toBe(res.filePath)
  })
})

// ─────────────── renaming keeps the Sound linked to its origin ───────────────
...
describe('renaming preserves author, License and Freesound linkage', () => {
  it('sets custom_name only — the sounds row (author / License / URL / name) is untouched', async () => {
    ...
    // inherited + own tags coexist and are distinguishable
...
describe('core.filterLibrary — every dimension, no gateway call', () => {
  ...
  it('filters by file format, License, duration, tag (inherited + custom), free text — and their combinations', async () => {
    ...
    // License — 'commercial' admits CC0 + CC-BY only
    ...
    // an empty filter is the whole Library, newest-saved first
    expect(core.filterLibrary({}).map((s) => s.id)).toEqual([
      600003, 600002, 600001,
    ])

    // NOT ONE gateway search happened for any of the above
  ...
  it('the persisted Library filter round-trips through a fresh core on the same DB', async () => {
    ...
    // normalised: tags trimmed + de-duped, text trimmed
...
// ──────────────── the pure predicate (eyeballed directly) ────────────────
...
describe('matchesLibraryFilter (pure)', () => {
  ...
  it('composes dimensions with AND and matches tags across inherited + custom', () => {
    ...
    expect(matchesLibraryFilter(base, { text: 'my name' })).toBe(true) // custom name
    expect(matchesLibraryFilter(base, { text: 'freesound' })).toBe(true) // fs name
    expect(matchesLibraryFilter(base, { text: 'author_x' })).toBe(true) // author
    expect(matchesLibraryFilter(base, { text: 'own-tag' })).toBe(true) // tag
    expect(matchesLibraryFilter(base, { license: 'commercial' })).toBe(true) // CC-BY
    expect(
