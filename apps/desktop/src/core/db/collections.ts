// `collections` + `collection_members` access (migration 001).
//
// A Collection is a user-named, unordered set of Library Sounds (CONTEXT.md §
// Collection). It does NOT nest, does NOT own its members, and has no existence
// on disk — it lives entirely here, per ADR-0002.
//
//   - A Sound may belong to any number of Collections at once
//     (`collection_members` is a plain many-to-many join, PK on the pair).
//   - Adding is idempotent (`INSERT … ON CONFLICT DO NOTHING`); a batch add of
//     several Sounds is one transaction.
//   - Removing a Sound from a Collection deletes ONLY the join row — the
//     `library_entries` row and every other Collection membership are untouched.
//   - Deleting a Collection deletes its `collections` row; its
//     `collection_members` rows go with it via `ON DELETE CASCADE` (foreign keys
//     are enabled in `db/index.ts`). `library_entries` is never touched.
//   - Deleting a Sound from the Library must also clear its memberships. The FK
//     from `collection_members.sound_id` to `sounds(id)` cascades, but
//     `deleteFromLibrary` keeps the `sounds` row, so the core calls
//     `clearSoundFromAllCollections` explicitly in that path.
