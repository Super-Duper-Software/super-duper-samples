// `DragHost` — the app's ONE boundary to the operating-system drag-and-drop
// call. In production this is `webContents.startDrag` (wired in `src/main/`); in
// tests it is `createRecordingDragHost`, which just records what it was handed.
// Ticket 09 exists to make "what the app hands to the OS" observable without
// launching Electron (CONVENTIONS.md § the load-bearing rule).
