// `staged_entries` access (migrations 001 + 002).
//
// A row here means: a complete Original for this Sound is on disk in the content
// store, and the Sound has NO `library_entries` row (it is Staged, not saved).
// Ticket 10 reads these rows — `byte_size` + `last_access_at`, LRU-first — to
// enforce the staging byte budget. Ticket 08 only WRITES them:
//
//   - `upsertStagedEntry` when an Original lands (or is re-staged);
//   - `touchStagedEntry` on every later audition of an already-staged Sound.
//
// `created_at` is set once and never moved; `last_access_at` is bumped on every
// touch. Eviction (ticket 10) must skip any Sound that also has a
// `library_entries` row — see `hasLibraryEntry`.
