// Headless audio decoding for peak computation (ticket 12).
//
// Freesound Originals are wav / aiff / flac / mp3 / ogg. This decoder handles the
// two UNCOMPRESSED PCM containers — WAV (RIFF, little-endian) and AIFF/AIFC
// (big-endian) — which is the case the ticket foregrounds: a long uncompressed
// field recording whose fixed-resolution Freesound waveform image is too coarse
// to inspect. Every other container throws `UndecodableAudioError`, which the
// caller turns into a graceful fall-back to the Freesound waveform image
// (acceptance: "audio that cannot be decoded falls back to its waveform image").
//
// No DOM `AudioContext`, no Electron, no native addon — pure byte parsing — so it
// runs unchanged in a Vitest process and inside a `node:worker_threads` Worker.
//
// A compressed-format decoder (`audio-decode` or similar wasm) can be slotted in
// behind this same `decodeAudioBuffer` seam later; see PROGRESS.md ticket 12
// deferrals.
...
const MAX_FRAMES = 2_000_000_000 // guard against a bogus header claiming a huge length
...
function decodeWav(view: DataView, bigEndian: boolean): DecodedAudio {
  ...
  while (p + 8 <= view.byteLength) {
    ...
    if (id === 'fmt ') {
      ...
      // WAVE_FORMAT_EXTENSIBLE: the real format is the first 2 bytes of the GUID.
      ...
      dataOffset = body
      // A streamed file may store 0 / 0xffffffff — clamp to what is actually present.
...
function decodeAiff(view: DataView): DecodedAudio {
  ...
  // AIFF PCM is big-endian, signed, left-justified in its byte width.
...
function pcmSampleReader(
  ...
  switch (bits) {
    case 8:
      // 8-bit PCM is unsigned (0..255) in WAV; AIFF 8-bit is signed. WAV is the
      // only 8-bit path that reaches here in practice.
