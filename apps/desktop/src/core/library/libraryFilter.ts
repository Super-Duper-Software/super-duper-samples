// Ticket 13 — the pure Library filter predicate. No I/O, no Electron: it takes a
// fully-hydrated `LibrarySound` (Sound + the user's overlay) and a structured
// `LibraryFilter` and answers yes/no. `core.filterLibrary` reads the whole
// Library from SQLite once and runs every row through this — there is never a
// network request, and the Library is a small, per-device table so a linear pass
// is instant.
//
// Dimensions compose with AND. `tags` matches a Sound carrying ANY of the listed
// tags (inherited from Freesound OR added by the user). `text` is a
// case-insensitive substring match across the custom name, the Freesound name,
