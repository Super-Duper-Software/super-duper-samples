// Pure region-select geometry for the Edit view (ticket 07). A **region** is a
// `[start, end]` pair of fractions of the whole file, each in [0, 1], with
// `start < end`. This module owns every piece of math that turns a pointer
// event into a region and a region into displayable seconds — no DOM, no
// React, mirroring `waveformPeaks.ts`'s pure-helper shape (prior art
// `test/waveform-peaks.test.ts`).
