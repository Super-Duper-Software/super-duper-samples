// The download queue (ticket 08). A plain, injectable module: it owns the
// concurrency cap, per-sound cancellation, and transient-failure retry with
// backoff. It knows nothing about SQLite, the content store, auth, or the
// renderer — those come in as callbacks so the queue stays unit-testable on its
// own.
//
// Lifecycle of one sound id:
//
//   enqueue()  ─→  queued  ─→  downloading  ─→  ready       (onComplete resolved)
//                                │   │
//                                │   └─ transient error, attempts left
//                                │        → back to queued after backoff
//                                └─ attempts exhausted        → failed
//
//   cancel()  removes a queued job outright, or aborts an in-flight one; either
//   way the job is dropped and its status reverts to `not-started`.
...
export function createDownloadQueue(deps: DownloadQueueDeps): DownloadQueue {
  ...
  const pending: number[] = [] // fifo of queued sound ids
  let running = 0
  ...
  function enqueue(soundId: number): void {
    ...
    if (existing) {
      if (existing.status === 'failed') {
        // A fresh audition of a previously-failed sound gets one more full run.
        drop(soundId)
      } else {
  ...
  function cancel(soundId: number): void {
    const job = jobs.get(soundId)
    if (!job) return
    // `drop` aborts the job. If it was in flight, `runAttempt` observes the abort,
    // releases its concurrency slot in `finally`, and re-pumps — so we must NOT
    // touch `running` here or it would be double-decremented.
    drop(soundId)
  ...
  async function runAttempt(job: Job): Promise<void> {
    ...
    try {
      ...
      if (job.abort.signal.aborted || isAbortError(err) || jobs.get(job.soundId) !== job) {
        return // cancelled — cancel() already emitted, job already dropped
      }
      if (job.attempts < maxRetries) {
        const wait = backoff[Math.min(job.attempts, backoff.length - 1)] ?? 0
        job.attempts += 1
        // Free the concurrency slot during the wait, then requeue.
        releaseSlot()
        job.status = 'queued'
      ...
      // Retries exhausted — surface a permanent failure and STOP. The job stays
      // in the map with `failed` so `status()` keeps reporting it until the user
      // auditions the sound again.
      job.status = 'failed'
