// Ticket 16 — Collections, at the core seam. No Electron: real temp SQLite, real
// temp filesystem, the fake gateway (and a gateway whose every method rejects
// for the "served from the DB, no network" assertions).
//
// Covers the ticket's "Tests cover:" line:
//   - a Sound can belong to multiple Collections at once
//   - removing a Sound from a Collection preserves Library membership AND its
//     membership of every other Collection
//   - deleting a Collection leaves all its Sounds in the Library untouched
//   - deleting a Sound from the Library clears every Collection membership
//   - batch add (several selected Sounds in one action) works and is idempotent
// plus: create / rename; file-at-save-time via `saveToLibrary`; membership
// persists across a restart; `listCollectionSounds` makes no gateway call and
// carries the user's overlay; `getCollectionsForSounds` badge shape.
...
describe('creating and renaming Collections', () => {
  it('creates a named Collection, visible in listCollections with a zero count', async () => {
    ...
    expect(made.name).toBe('Weather') // trimmed
...
// ───────────── a Sound belongs to any number of Collections ─────────────
...
describe('several selected Sounds are added in one action', () => {
  it('batch-adds a mixed selection, skipping the ones already in the Collection', async () => {
    ...
    // 1 is already in; 2,3,4 are new — one call.
...
describe('deleting a Collection', () => {
  it('leaves all its Sounds in the Library untouched', async () => {
    ...
    expect(core.listCollections()).toEqual([])
    // both Sounds still in the Library, files still on disk
...
describe('deleting a Sound from the Library', () => {
  it('removes it from every Collection it belonged to', async () => {
    ...
    // the other Sound's memberships are intact
...
describe('Collections are served from the database', () => {
  it('listCollectionSounds makes no gateway call and carries the user overlay', async () => {
    ...
    // FakeFreesoundGateway counts search calls; a dead gateway would reject.
    expect(
