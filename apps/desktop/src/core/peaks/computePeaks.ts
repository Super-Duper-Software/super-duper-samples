// Peak reduction (ticket 12). Turns decoded PCM into a compact min/max envelope
// the renderer draws to a <canvas> at any width and zoom level.
//
// The envelope is computed ONCE at a fixed base resolution and cached in SQLite.
// The renderer downsamples further per pixel column for narrow widths, and for a
// zoom it draws a sub-range of the same buckets — so no recompute is ever needed
// to resize or zoom (only the source audio changing would invalidate it, and an
// Original never changes in place).
 ...
 * an accepted v1 limit (PROGRESS.md ticket 12).
 */
...
export function computePeaks(
  ...
  for (let b = 0; b < bucketCount; b++) {
    ...
    // A silent bucket has min > max after the sweep; normalise it to a flat line.
    if (min > max) {
      min = 0
      max = 0
    }
